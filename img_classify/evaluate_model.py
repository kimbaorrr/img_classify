...
class EvalofTraining:
	...
	def __init__(
		...
		self.epochs = len(self.history['loss'])
		self.len_of_epochs = range(1, self.epochs + 1)

		plt.title('Loss')
		plt.plot(self.len_of_epochs, self.history['loss'], 'ro', label='loss')
		plt.plot(self.len_of_epochs, self.history['val_loss'], 'b', label='val_loss')
		if max(self.len_of_epochs) <= 30:
			plt.xticks(self.len_of_epochs)
		plt.xlabel('Epochs')
		...
		plt.title('Accuracy')
		plt.plot(self.len_of_epochs, self.history['accuracy'], 'ro', label='accuracy')
		plt.plot(self.len_of_epochs, self.history['val_accuracy'], 'b', label='val_accuracy')
		if max(self.len_of_epochs) <= 30:
			plt.xticks(self.len_of_epochs)
		plt.xlabel('Epochs')
		plt.ylabel('% (Percentage)')
